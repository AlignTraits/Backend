  ...
  for (let i = 0; i < schools.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const result = await db.$transaction(
        async (tx) => {
          ...
          for (const school of batch) {
            try {
              ...
              batchFailures.push({ school, error: userFriendlyMessage });
            }
          }
        ...
        {
          timeout: 120000, // This now applies to the entire batch!
          maxWait: 120000,
        }
      );

      successfulSchools.push(...result.batchSuccess);
      failedSchools.push(...result.batchFailures);
    } catch (batchError) {
      // If the entire transaction fails, mark all schools in batch as failed
      for (const school of batch) {
        failedSchools.push({
          school,
...
// Helper function for error messages (you might already have this)
const getFriendlyErrorMessageUpdate = (
  error: any,
  ...
  if (error.message.includes('Missing school ID')) {
    ...
    return `School "${schoolIdentifier}" not found`;
  } else if (error.code === 'P2002') {
    return `School "${schoolIdentifier}" already exists with similar data`;
  } else {
    return `An unexpected error occurred while updating school "${schoolIdentifier}"`;
  }
};
...
export const updateBulkSchoolsService = async (
  ...
  for (let i = 0; i < schools.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const result = await db.$transaction(
        async (tx) => {
          ...
          for (const school of batch) {
            try {
              ...
              if (
                JSON.stringify(updateData) === JSON.stringify(existingSchool)
              ) {
                batchSkipped.push({
                  id: school.id,
                  name: existingSchool.name,
              ...
              const userFriendlyMessage = getFriendlyErrorMessageUpdate(
                error,
        ...
        {
          timeout: 120000, // 60 second timeout for entire batch
          maxWait: 120000,
        }
      );
      ...
      for (const school of batch) {
        failedUpdates.push({
          school,
          error: 'Batch processing failed - transaction timeout or error',
        });
      }
    }
  ...
  // Batch create failure logs for failed updates
  if (failedUpdates.length > 0) {
    await db.bulkOperationFailure.createMany({
      data: failedUpdates.map((failure) => ({
        entity: 'School',
        operation: 'Update',
        itemData: failure.school,
