...
// working well before adding jambCutOffMark
...
// older calculation not working perfect
// async function calculateServerEligibility(
//   userId: string,
//   input: ServerQualificationInput
// ): Promise<WetrocloudResponse> {
...
// working well After adding jambCutOffMark
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (['UTME', 'JAMB'].includes(examType.toUpperCase())) {
      utme.push({
        ...
        subjects,
        grades,
        jambCutOffMark: course.jambCutOffMark,
      });
    } else if (['WAEC', 'NECO', 'GCE'].includes(examType.toUpperCase())) {
      olevel.push({ examType, subjects, grades });
    }
...
function matchExamEligibility(
  ...
  // For JAMB/UTME, compare aggregate score with jambCutOffMark
  if (isUtme && required.jambCutOffMark !== undefined) {
    // Check if grades are numeric (JAMB scores)
    const allNumeric = studentExam.grades.every(
      (grade) => !isNaN(parseFloat(grade))
    );
    let studentAggregate: number;
    if (allNumeric) {
      // Sum numeric scores directly
      studentAggregate = studentExam.grades.reduce(
        (sum, grade) => sum + parseFloat(grade),
        0
      );
    } else {
      // Use grade points for letter grades
    ...
    // Compare with jambCutOffMark
    if (studentAggregate < required.jambCutOffMark) {
      return {
        eligible: false,
        details: `Insufficient JAMB aggregate: got ${studentAggregate}, required ${required.jambCutOffMark} or higher`,
      };
    } else {
      return {
        eligible: true,
        details: `JAMB aggregate met: got ${studentAggregate}, required ${required.jambCutOffMark} or higher`,
      };
    }
  }
  ...
  // Check if lengths match for non-JAMB/UTME
  if (
    required.subjects.length !== studentExam.subjects.length ||
    required.subjects.length !== studentExam.grades.length ||
    studentExam.subjects.length !== studentExam.grades.length
  ...
  // Check if all required subjects are present and validate grades for non-JAMB/UTME
  for (let i = 0; i < required.subjects.length; i++) {
    const reqSubject = required.subjects[i].toLowerCase();
    const reqGrade = required.grades[i];
