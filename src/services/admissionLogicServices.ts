...
export const updateCourseAdmissionService = async ({
  ...
  jambCutOffMark,
  userId,
}: UpdateCourseAdmissionData & { userId: string; jambCutOffMark?: number }) => {
  try {
    const existingCourse = await db.course.findUnique({ where: { id } });
    if (!existingCourse) {
      return { ok: false, status: 404, message: 'Course not found' };
    ...
    const updatedCourse = await db.course.update({
      ...
      data: {
        ...
        Adminrule4: Adminrule4 ?? existingCourse.Adminrule4,
        Adminrule5: Adminrule5 ?? existingCourse.Adminrule5,
        jambCutOffMark: jambCutOffMark ?? existingCourse.jambCutOffMark,
      },
    });
  ...
  for (let i = 0; i < admissions.length; i += BATCH_SIZE) {
    ...
    try {
      const results = await Promise.allSettled(
        batch.map(async (admission) => {
          let existingCourse: any = null;
          try {
            if (!admission.id) throw new Error('Missing course ID');
            existingCourse = await withTimeout(
              db.course.findUnique({ where: { id: admission.id } }),
              120000,
              `Timeout finding course with ID "${admission.id}"`
            );
            if (!existingCourse)
              throw new Error(`Course with ID ${admission.id} not found`);
            ...
            if (admission.jambCutOffMark !== undefined) {
              hasChanges = true;
              updateData.jambCutOffMark =
                admission.jambCutOffMark ?? existingCourse.jambCutOffMark;
            }
            ...
            const updatedCourse = await withTimeout(
              ...
              120000,
              `Timeout updating course with ID "${admission.id}"`
            );
            return { id: updatedCourse.id, title: updatedCourse.title };
          } catch (error: any) {
            return {
              admission,
              error: getFriendlyAdmissionErrorMessage(
                error,
                admission.id,
                existingCourse?.title
              ),
      ...
      const batchSuccessful = results
        .filter(
          (
            ...
            result.status === 'fulfilled' && !('message' in result.value)
        )
      ...
      const batchFailed = results
        .filter(
          (
            ...
            | PromiseFulfilledResult<{ admission: any; error: string }> =>
        ...
        .map((result) =>
          result.status === 'fulfilled'
            ? result.value
            : {
                admission: batch[results.indexOf(result)],
                error: result.reason.message,
              }
        );

      successfulUpdates.push(...batchSuccessful);
      failedUpdates.push(...batchFailed);
      skippedUpdates.push(...batchSkipped);
    } catch (batchError) {
      // Handle unexpected batch-level errors (e.g., database disconnection)
      for (const admission of batch) {
        failedUpdates.push({
          admission,
